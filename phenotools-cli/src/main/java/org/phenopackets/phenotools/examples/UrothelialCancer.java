...
class UrothelialCancer implements PhenopacketExample {
    ...
    private Disease infiltratingUrothelialCarcinoma() {
        return DiseaseBuilder.builder("NCIT:C39853", "Infiltrating Urothelial Carcinoma")
                ...
                .diseaseStage(of("NCIT:C27971", "Stage IV"))
                ...
                .clinicalTnmFinding(of("NCIT:C48766", "pT2b Stage Finding"))
                //pN2 Stage Finding (Code C48750)
                // cancer has spread to 2 or more lymph nodes in the true pelvis (N2)
                .clinicalTnmFinding(of("NCIT:C48750", "pN2 Stage Finding"))
                // M1 Stage Finding
                // the tumour has spread from the original site (Metastatic Neoplasm in lymph node - sample5)
                .clinicalTnmFinding(of("NCIT:C48700", "M1 Stage Finding"))
                .build();
    }

    private Biosample prostateBiosample() {
        OntologyClass prostateGland = of("UBERON:0002367", "prostate gland");
        OntologyClass prostateAcinarAdenocarcinoma = of("NCIT:C5596", "Prostate Acinar Adenocarcinoma");
        return BiosampleBuilder.builder("prostate biosample ID")
                ...
                .tumorProgression(of("NCIT:C95606", "Second Primary Malignant Neoplasm"))
                .tumorGrade(of("NCIT:C28091", "Gleason Score 7"))
                .procedure(ProcedureBuilder.builder(BIOPSY).build())
                .build();
    }

    private Biosample leftUreterBiosample() {
        OntologyClass leftUreter = of("UBERON:0001223", "left ureter");
        return BiosampleBuilder.builder("left ureter biosample ID")
                .sampledTissue(leftUreter)
                .individualId(PROBAND_ID)
                .timeOfCollection(AGE_AT_BIOPSY)
                .histologicalDiagnosis(of("NCIT:C38757", "Negative Finding"))
                .procedure(ProcedureBuilder.builder(BIOPSY).build())
                .build();
    }
    ...
    private Biosample rightUreterBiosample() {
        ...
        OntologyClass rightUreter = of("UBERON:0001222", "right ureter");
    ...
    private Biosample pelvicLymphNodeBiosample() {
        OntologyClass pelvicLymphNode = of("UBERON:0015876", "pelvic lymph node");
    ...
    private Biosample bladderBiosample() {
        OntologyClass bladderWall = of("UBERON_0001256", "wall of urinary bladder");
